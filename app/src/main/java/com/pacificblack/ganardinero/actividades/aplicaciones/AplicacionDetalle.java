...
public class AplicacionDetalle extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (objetoAplicaciones != null) {
            ...
            if (aplicaciones.getDes2().equals(Vacio)){des2.setVisibility(View.GONE);}else {des2.setText(aplicaciones.getDes2());}
            if (aplicaciones.getDes3().equals(Vacio)){des3.setVisibility(View.GONE);}else {des3.setText(aplicaciones.getDes3());}
            if (aplicaciones.getDes4().equals(Vacio)){des4.setVisibility(View.GONE);}else {des4.setText(aplicaciones.getDes4());}
            if (aplicaciones.getDes5().equals(Vacio)){des5.setVisibility(View.GONE);}else {des5.setText(aplicaciones.getDes5());}

            if (aplicaciones.getImagen2().equals(Vacio)){imagen2.setVisibility(View.GONE);}else {Picasso.get().load(aplicaciones.getImagen2()).placeholder(R.drawable.sample).error(R.drawable.sample).into(imagen2); }
            if (aplicaciones.getImagen3().equals(Vacio)){imagen3.setVisibility(View.GONE);}else {Picasso.get().load(aplicaciones.getImagen3()).placeholder(R.drawable.sample).error(R.drawable.sample).into(imagen3); }
            if (aplicaciones.getImagen4().equals(Vacio)){imagen4.setVisibility(View.GONE);}else {Picasso.get().load(aplicaciones.getImagen4()).placeholder(R.drawable.sample).error(R.drawable.sample).into(imagen4); }
            if (aplicaciones.getImagen5().equals(Vacio)){imagen5.setVisibility(View.GONE);}else {Picasso.get().load(aplicaciones.getImagen5()).placeholder(R.drawable.sample).error(R.drawable.sample).into(imagen5); }
