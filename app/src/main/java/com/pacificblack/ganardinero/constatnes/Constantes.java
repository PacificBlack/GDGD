package com.pacificblack.ganardinero.constatnes;

public class Constantes {
    public static String IdAnuncioAplicaciones = "ca-app-pub-3940256099942544/1033173712";
    public static String IdAnuncioVideos = "ca-app-pub-3940256099942544/1033173712";
    public static String IdAnuncioMineria = "ca-app-pub-3940256099942544/1033173712";
    public static String IdAnuncioDesarrollo = "ca-app-pub-3940256099942544/1033173712";
    public static String IdAnuncioPublicidad = "ca-app-pub-3940256099942544/1033173712";
    public static String IdAnuncioVender = "ca-app-pub-3940256099942544/1033173712";
    public static String IdAnuncioWorkHome = "ca-app-pub-3940256099942544/1033173712";


    public static String Vacio = "Vacio";
    public static String Imagen = "imagen";

    public static String DBAplicaciones = "aplicaciones";
    public static String DBMineria = "mineria";
    public static String DBVideos = "videos";
    public static String DBDesarrollo = "desarrollo";
    public static String DBPublicidad = "publicidad";
    public static String DBVender = "vender";
    public static String DBWorkHome = "workhome";


    public static String Ftitulo = "titulo";
    public static String Fdescripcion1 = "des1";
    public static String Fdescripcion2 = "des2";
    public static String Fdescripcion3 = "des3";
    public static String Fdescripcion4 = "des4";
    public static String Fdescripcion5 = "des5";
    public static String Ffecha = "fecha";
    public static String Fimagen1 = "imagen1";
    public static String Fimagen2 = "imagen2";
    public static String Fimagen3 = "imagen3";
    public static String Fimagen4 = "imagen4";
    public static String Fimagen5 = "imagen5";
}
